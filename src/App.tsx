...
export default function App() {
  ...
  useEffect(() => {
    const checkIpBan = async () => {
      try {
        ...
        if (clientData.ip) {
          const banDoc = await getDoc(doc(db, 'banned_ips', clientData.ip));
          if (banDoc.exists()) {
            setIsBanned(true);
          }
        }
  ...
  if (isBanned && !isAdmin) {
    ...
    content = <HomePage onNavigate={setPage} onLogin={handleGoogleSignIn} isAdmin={isAdmin} profile={profile} user={user} onSecretClick={() => setShowAdminModal(true)} />;
  } else if (page === 'vote') {
          ...
          >
            ...
            >
              ...
              {!user && (
                <button
                  onClick={handleGoogleSignIn}
                  className="w-full flex items-center justify-center gap-3 bg-white text-gray-900 px-6 py-3 rounded-xl font-bold hover:bg-gray-100 transition-all mb-6 shadow-lg"
                >
                  <svg className="w-5 h-5" viewBox="0 0 24 24">
                    <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4" />
                    <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853" />
